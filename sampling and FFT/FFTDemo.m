...
f = 50; %cycles per second (Hz)
% f = 30; %cycles per second (Hz)
f1 = 20;
f2 = 30;
f3 = 40;
...
x0 = 1 * cos(2 * pi * f * t + pi/2); %sinusoidal wave
...
% bins with frequency components: X_Mag
...
% basically saying we want fs number of equally spaced values between -1, 1
...
subplot(3, 1, 2)
plot(f_axis, 20 * log10(X_Shift/(N/2)))
ylabel('amplitude (dB)')
xlabel('frequency (Hz)')
grid on

X_PhaseShift = fftshift(X_phase);
subplot(3, 1, 3)
plot(f_axis, X_PhaseShift / pi * 180)
